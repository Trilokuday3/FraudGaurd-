...
def _log_deployed_run(mlflow_dir, data_dir, experiment_name, run_name="candidate"):
    """Train a real (tiny) LogisticRegression and log it in the shape
    promote_model expects: calibrated_deployed_model + raw_deployed_model,
    same as ml/__main__.py's real deployed_model_final run."""
...
def test_promote_model_first_promotion_has_no_previous(tmp_path):
    data_dir = _tiny_data_dir(tmp_path / "data")
    mlflow_dir = str(tmp_path / "mlruns")
    run_id = _log_deployed_run(mlflow_dir, data_dir, "test-promote-1")
    ...
    result = promote_model(
        ...
        registered_model_name="test-model-1",
    )

    assert result["promoted_run_id"] == run_id
    assert result["previous_champion_run_id"] is None

    client = MlflowClient()
    champion = client.get_model_version_by_alias("test-model-1", "champion")
    assert champion.run_id == run_id

    assert os.path.exists(tmp_path / "thresholds.json")
    assert (tmp_path / "store").is_dir()


def test_promote_model_second_promotion_sets_previous_alias(tmp_path):
    data_dir = _tiny_data_dir(tmp_path / "data")
    mlflow_dir = str(tmp_path / "mlruns")

    run_id_1 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-2", run_name="first")
    promote_model(
        run_id_1,
        mlflow_tracking_uri=mlflow_dir,
        data_dir=data_dir,
        model_store_dir=str(tmp_path / "store"),
        thresholds_path=str(tmp_path / "thresholds.json"),
        registered_model_name="test-model-2",
    )

    run_id_2 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-2", run_name="second")
    result = promote_model(
        run_id_2,
        mlflow_tracking_uri=mlflow_dir,
        data_dir=data_dir,
    ...
    assert result["previous_champion_run_id"] == run_id_1

    client = MlflowClient()
    champion = client.get_model_version_by_alias("test-model-2", "champion")
    previous = client.get_model_version_by_alias("test-model-2", "previous")
    assert champion.run_id == run_id_2
    assert previous.run_id == run_id_1


def test_promote_model_refuses_a_candidate_that_does_not_beat_champion(tmp_path, monkeypatch):
    data_dir = _tiny_data_dir(tmp_path / "data")
    mlflow_dir = str(tmp_path / "mlruns")

    run_id_1 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-3", run_name="first")
    promote_model(
        run_id_1,
        mlflow_tracking_uri=mlflow_dir,
        data_dir=data_dir,
        model_store_dir=str(tmp_path / "store"),
        thresholds_path=str(tmp_path / "thresholds.json"),
        registered_model_name="test-model-3",
    )

    run_id_2 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-3", run_name="second")

    import mlops.promote_model as promote_module

    real_enrich = promote_module.enrich_deployed_run

    def fake_enrich(run_id, **kwargs):
        result = real_enrich(run_id, **kwargs)
        if run_id == run_id_2:
            result["deployed_val_pr_auc"] = -1.0
        return result

    monkeypatch.setattr(promote_module, "enrich_deployed_run", fake_enrich)
    ...
    client = MlflowClient()
    champion = client.get_model_version_by_alias("test-model-3", "champion")
    assert champion.run_id == run_id_1


def test_promote_model_force_bypasses_the_comparison(tmp_path, monkeypatch):
    data_dir = _tiny_data_dir(tmp_path / "data")
    mlflow_dir = str(tmp_path / "mlruns")

    run_id_1 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-4", run_name="first")
    promote_model(
        run_id_1,
        mlflow_tracking_uri=mlflow_dir,
        data_dir=data_dir,
        model_store_dir=str(tmp_path / "store"),
        thresholds_path=str(tmp_path / "thresholds.json"),
        registered_model_name="test-model-4",
    )

    run_id_2 = _log_deployed_run(mlflow_dir, data_dir, "test-promote-4", run_name="second")
    ...
    assert result["promoted_run_id"] == run_id_2
    client = MlflowClient()
    champion = client.get_model_version_by_alias("test-model-4", "champion")
    assert champion.run_id == run_id_2
